class Fleet {
  ...
  void updateLasers() {
    ...
    for (Laser l : laserToRemove) {
      laser.laserExplotion(l.location.x, l.location.y, lasers, l);
    }
  }
  ...
  void maneuveFleet() {
    speedUpdate -= 3;
    speedUpdate = constrain(speedUpdate, 3, 40);
    if (velocityX > 0) {
  ...
  void display() {
    for (SpaceInvader spaceInvader : invaders) {
      spaceInvader.display(positionFlag, false);
    }
    for (Laser l : lasers) {
      l.display();
    }
