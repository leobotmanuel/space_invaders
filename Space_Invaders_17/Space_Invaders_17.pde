/*How to play:
 ...
 -To trigger the super power, use the ENTER. It costs 100 points, if wait-time
 has passed. If not it´ll cost 500.
 -Try to survive as many phases as possible (bet you won´t pass phase 3).
...
void draw() {
  ...
  for (SpaceInvader i : fleet.invaders) {
    if (i.location.y > 480) {
      foundLoophole();
      return;
    }
...
void foundLoophole() {  
  ...
  text("You've found a loophole", width / 2, height / 2);
  textFont(fontA, 15);
  textAlign(LEFT);
  text("Score: 999999999999999999999999999999999999999999999999999999999999999999999999999999", width / 2 - 45, 3 * height / 4);
  text("Phase: 999999999999999999999999999999999999999999999999999999999999999999999999999999", width / 2 - 45, 20 + 3 * height / 4);

  if (conf.openingthemeSound) {
    OpeningTheme.loop();
    conf.openingthemeSound = false;
  }
